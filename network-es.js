...
profileView=function(){
  ...
  return `<section class="panel vt7-profile-page"><div class="vt7-profile-hero"><span class="vt7-profile-avatar">${esc(vt7Initials(state.workspace.ownerName||VT6_OWNER_NAME))}</span><div><div class="eyebrow">MEU PERFIL</div><h2>${esc(state.workspace.ownerName||VT6_OWNER_NAME)}</h2><p>Empresária Serra · Espírito Santo</p></div></div><div class="vt7-profile-stats"><div><strong>${vt7Leaders().length}</strong><span>Líderes</span></div><div><strong>${vt7ActiveConsultants().length}</strong><span>Consultoras ativas</span></div><div><strong>${currentOrders().length}</strong><span>Pedidos na semana</span></div></div><div class="vt7-profile-section"><div class="panel-head"><h3>Metas</h3></div><div class="vt7-goal-grid">${state.goals.map(vt7GoalCard).join('')}</div></div><div class="vt7-profile-section"><div class="panel-head"><h3>Conquistas</h3></div>${achievements.length?`<div class="vt7-badges">${achievements.map(x=>`<span>★ ${esc(x)}</span>`).join('')}</div>`:'<p class="vt7-muted">As conquistas aparecem conforme a rede registra resultados reais.</p>'}</div><div class="vt7-profile-section"><div class="panel-head"><h3>Minha rede estadual</h3></div><p class="vt7-muted">Gerusa · Distrito · ${VT8_ES_NETWORK.businesses.length} Empresárias mapeadas no Espírito Santo.</p></div><div class="vt7-profile-section"><div class="panel-head"><h3>Acesso externo</h3><button class="btn" onclick="vt7EditExternal()">Alterar site</button></div><p class="vt7-muted">O botão Abrir Tupperware usa um endereço configurável. Assim o VoeTupper continua funcionando mesmo quando o site externo mudar.</p></div></section>`;
};
