...
create table public.campaigns(id uuid primary key default gen_random_uuid(),workspace_id uuid not null references public.workspaces on delete cascade,label text not null,created_at timestamptz not null default now(),unique(workspace_id,id));
create table public.weeks(id uuid primary key default gen_random_uuid(),workspace_id uuid not null references public.workspaces on delete cascade,campaign_id uuid not null,label text not null,starts_at timestamptz,closes_at timestamptz,team_goal numeric(12,2) not null default 0 check(team_goal>=0),status public.week_status not null default 'PLANNED',unique(workspace_id,id),foreign key(workspace_id,campaign_id) references public.campaigns(workspace_id,id) on delete restrict);
create table public.orders(id uuid primary key default gen_random_uuid(),workspace_id uuid not null references public.workspaces on delete cascade,consultant_id uuid not null,week_id uuid not null,source public.source_channel not null,summary text not null check(char_length(summary) between 1 and 500),amount numeric(12,2) check(amount>=0),stage public.order_stage not null default 'RECEIVED',created_by uuid references auth.users,created_at timestamptz not null default now(),updated_at timestamptz not null default now(),foreign key(workspace_id,consultant_id) references public.consultants(workspace_id,id) on delete restrict,foreign key(workspace_id,week_id) references public.weeks(workspace_id,id) on delete restrict);
create table public.offers(id uuid primary key default gen_random_uuid(),workspace_id uuid not null references public.workspaces on delete cascade,week_id uuid,campaign_id uuid,title text not null,active boolean not null default true,check(week_id is not null or campaign_id is not null),foreign key(workspace_id,week_id) references public.weeks(workspace_id,id) on delete cascade,foreign key(workspace_id,campaign_id) references public.campaigns(workspace_id,id) on delete cascade);
create table public.recognitions(id uuid primary key default gen_random_uuid(),workspace_id uuid not null references public.workspaces on delete cascade,week_id uuid not null,consultant_id uuid not null,label text not null,confirmed_at timestamptz,confirmed_by uuid references auth.users,foreign key(workspace_id,week_id) references public.weeks(workspace_id,id) on delete cascade,foreign key(workspace_id,consultant_id) references public.consultants(workspace_id,id) on delete cascade);
...
-- Workspace creation is mediated by a trusted server/RPC so creator membership is atomic.
